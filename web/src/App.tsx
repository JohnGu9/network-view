...
import { Button, Card, CircularProgress, Elevation, Icon, IconButton, ListItem, Tab, TabBar, Theme, Typography } from 'rmcw'
import Connection, { DataType, HeaderType, InterfaceDataType } from './common/Connection';
...
function ConnectionWaiting({ children }: { children: React.ReactNode }) {
  ...
  const [connection, setConnection] = React.useState(null as null | DataManager);
  React.useEffect(() => {
    if (ws !== null) {
      const connection = new Connection(ws);
      const manager = new DataManager(connection, (d) => setData(d));
  ...
  return (
    <SharedAxis keyId={connection === null ? 0 : 1}
      className='full-size row flex-stretch'>
      {connection === null || data === null ?
...
function toChartOption(output: ([number, number])[], input: ([number, number])[]): echarts.EChartsOption {
  return {
    grid: {
      top: 42,
      left: 20,
    ...
    tooltip: {
      ...
      formatter: function (params) {
        const vals = (params as echarts.DefaultLabelFormatterCallbackParams[]).reduce(
          (prev, curr) =>
  ...
  const { output, input } = React.useMemo(() => {
    ...
    for (const [timestamp, d] of data.history) {
      ...
      for (const [key, v] of Object.entries(d)) {
        let h = JSON.parse(key) as HeaderType;
...
function IpAddress({ data }: {
  ...
  const { ips, timestamps } = React.useMemo(() => {
    ...
    const timestamps = data.history.map(([t, _]) => t);
